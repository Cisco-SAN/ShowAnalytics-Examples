...
def validateArgs (args) :
    ...
    if args.alias :
        if not (args.initiator_itl or args.target_itl):
            print "\n Alias option is only supported with initiator_itl, target_itl\n"
            return False
    if args.lun :
...
def displayOutstandingIo(args,json_out,return_vector) :
...
parser.add_argument('--version', action='version', help='version', version='%(prog)s 2.0')
...
if not json_out and not args.evaluate_npuload:
    ...
    if args.errors or args.errorsonly:
        ...
        return_vector = displayOutstandingIo(args,json_out,[None,1,None])
        if args.refresh :
            while (not(return_vector[0] == None  and return_vector[2] == None) ) :
                json_out = getData(args)
                if not json_out :
                    json_out = ' '
                return_vector = displayOutstandingIo(args,json_out,return_vector)
